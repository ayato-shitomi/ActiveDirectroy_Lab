...
$config = @{
    ...
    UserPwdHasegawa = "${user_password_hasegawa}"
    UserPwdSaitou = "${user_password_saitou}"
    SvcBackupPwd = "${svc_backup_password}"
    FlagDcAdmin = "${flag_dc_admin}"
}
...
for($retry = 1; $retry -le 3; $retry++) {
    try {
        ...
        Invoke-WebRequest -Uri "$baseUrl/scripts/dc/setup.ps1" -OutFile "$ScriptPath\setup.ps1" -UseBasicParsing -TimeoutSec 30 -MaximumRedirection 5
        Write-Host "Downloaded setup.ps1"

        # Verify files exist and have content
        if((Test-Path "$ScriptPath\setup.ps1") -and ((Get-Item "$ScriptPath\setup.ps1").Length -gt 0)) {
